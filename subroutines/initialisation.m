...
if opt.part2
    HT = @(x) Hadamard2Dtranspose_01(x, M, N, Index_random_full(N / 2 + 1 : end, :) ); 
    H = @(x) Hadamard2D_01(x, M, N, Index_random_full(N / 2 + 1 : end, :) );
else
    HT = @(x) Hadamard2Dtranspose_01(x, M, N, Index_random_full); 
    H = @(x) Hadamard2D_01(x, M, N, Index_random_full);
end
